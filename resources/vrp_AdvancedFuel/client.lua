	...
	if(IsPedInAnyVehicle(GetPlayerPed(-1), -1) and GetPedVehicleSeat(GetPlayerPed(-1)) == -1 and not isBlackListedModel()) then
		...
		if(hud_form == 1) then
			...
			if(showText) then
				...
				drawTxt(UI.x + 0.62, 	UI.y + 1.31, 1.0,1.0,0.60 , round(percent,1).."", 255, 255, 255, 255)
				drawTxt(UI.x + 0.648, 	UI.y + 1.318, 1.0,1.0,0.4, "~w~ %", 255, 255, 255, 255)
			end
		else
			if(showBar) then
