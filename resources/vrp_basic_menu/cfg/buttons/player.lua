...
local armor_body_cd = {}
function guardarBodyBuilderPorra()
  for user_id,cd in pairs(armor_body_cd) do
    if cd > 0 then
      armor_body_cd[user_id] = cd - 1
    end
  end
  SetTimeout(1000,function()
    guardarBodyBuilderPorra()
  end)
end
...
choice_store_armor = {function(player, choice)
  local user_id = vRP.getUserId(player)
    if (armor_body_cd[user_id] == nil or armor_body_cd[user_id] == 0) and user_id ~= nil then
    armor_body_cd[user_id] = 5
		if user_id ~= nil then
