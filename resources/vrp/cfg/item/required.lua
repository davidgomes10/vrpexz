...
-- money
items["money"] = {"Money","Dinheiro Compactado.",function(args)
  local choices = {}
  local idname = args[1]
  ...
  choices["Unpack"] = {function(player,choice,mod)
    ...
    if user_id then
      ...
      local ramount = vRP.prompt(player, "Quanto você quer descompactar ? (max "..amount..")", "")
...
local wammo_choices = function(args)
  ...
  choices["Carregar"] = {function(player,choice)
    ...
    if user_id then
      ...
      local ramount = vRP.prompt(player, "Quanto quer recarregar? (max "..amount..")", "")
      ramount = parseInt(ramount)
