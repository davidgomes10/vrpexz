    ...
    choices[act] = {function(player,choice)
      ...
      if user_id ~= nil then
        if vRP.tryGetInventoryItem(user_id,idname,1,false) then
          if vary_hunger ~= 0 
		  then 
			local idMarginal = vRP.getUserId(player)
			local source = vRP.getUserSource(idMarginal)
			vRP.varyHunger(user_id,vary_hunger) 
			TriggerClientEvent("carhud:setHunger", source, vRP.getHunger(idMarginal))
		  end
          if vary_thirst ~= 0 
		  then 
			local idMarginal = vRP.getUserId(player)
			...
			TriggerClientEvent("carhud:setThirst", source, vRP.getThirst(idMarginal))
		   end
...
items["avaraqueraba"] = {"A vara queraba","", gen("eat",100,100),0.5}
